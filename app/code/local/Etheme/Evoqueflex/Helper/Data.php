...
class Etheme_Evoqueflex_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function createPreview($name,$pathModule,$id,$width,$height)
    {
        if(isset($_FILES[$name]['name']) && $_FILES[$name]['name'] != null)
        {
            ...
            if(empty($id))
            {
                $id = Mage::getModel('evoqueflex/evoqueflex')
                    ->getCollection()
    ...
    public function getPreview($slide_id, $direction='next')
    {
        ...
        $path= Mage::getBaseUrl('media').'etheme/evoque/evoqueflex/preview_'.$out.'.jpg';
        return $path;
    }
